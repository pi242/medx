...
class LimeVectorExplainer(object):
    ...
    def data_labels(self,
        ...
        data = self.random_state.randint(0, 2, num_samples * (n_features + 1))
        data = data.reshape((num_samples, (n_features + 1)))
        ...
        for d in data:
            ...
            if d[-1] == 0:
                stretched = self.align(temp, peaks)
                new_vec = self.CropData(stretched)
                # np.save('/home/pi242/xai/xai_results/lime_rr_res/' + 'aligned', stretched)
                imgs.append(new_vec)
            else:
                imgs.append(temp)
