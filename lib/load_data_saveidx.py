...
class Physionet2017Dataset(Dataset):
    ...
    def __init__(
        ...
        if kfolds > 1:
            ...
            if saveidx_dir != None:
                if not os.path.exists(saveidx_dir + '/trainidxs.npy'):
                    kf = StratifiedKFold(
                    ...
                    np.save(saveidx_dir + '/trainidxs', trainval_index)
                    np.save(saveidx_dir + '/testidxs', test_index)
                else:
                    print("idxs files already exist, loading...")
                    trainval_index = list(np.load(saveidx_dir + '/trainidxs.npy'))
                    test_index = list(np.load(saveidx_dir + '/testidxs.npy'))
                    if len(self.labels) in trainval_index:
                        trainval_index.remove(len(self.labels))
                    if len(self.labels) in test_index:
                        test_index.remove(len(self.labels)) 
            else:
                kf = StratifiedKFold(
                    n_splits=kfolds, shuffle=True, random_state=random_seed
                )
            ...
            if val_size == 0:
                if split_type == "train":
                    print(f'Trainval indices, length = {len(trainval_index)}')
                    # print('\n\n--')
                    # for ti in trainval_index:
                    #     print(ti)
                    # print('--\n\n')
                ...
                ss = sklearn.model_selectionStratifiedShuffleSplit(
    ...
    def load_data(self, idx):
        ...
        filename = ref + ".mat"
        # print(f'In load_data, filename = {filename}')
    ...
    def __getitem__(self, idx):
        ...
        ### Transform
        # print("In __getitem__, applying transform...")
        if self.transform:
...
class ToTensorWithPeak(object):
    ...
    def __call__(self, sample):
        ...
        if type(sample) is dict:
            data, label, peaks = sample["data"], sample["label"], sample["peaks"]
            # swap color axis because
            # numpy image: H x W x C
            # torch image: C X H X W
            for i in range(self.num_expand_dims):
            ...
            return {
                "data": torch.from_numpy(data).float(),
                "label": torch.from_numpy(np.array([label])).short(),
                "peaks": torch.from_numpy(np.array(peaks))
            }
...
class CropOnlyData(object):
    ...
    def __call__(self, sample):
        ...
        if len(sample) >= self.num_samples:
            start_idx = np.random.randint(len(sample) - self.num_samples + 1)
            sample = sample[start_idx : start_idx + self.num_samples]
        else:
            left_pad = int(np.ceil((self.num_samples - len(sample)) / 2))
            right_pad = int(np.floor((self.num_samples - len(sample)) / 2))
            sample = np.pad(sample, (left_pad, right_pad), "constant")

        return sample

class CropWithPeak(object):
    def __init__(self, num_samples, mode="constant"):
    ...
    def __call__(self, sample):
        ...
        # print("CropWithPeak called....")
        
        data, label, peaks = sample["data"], sample["label"], sample["peaks"]
        # print(f'In CropWithPeak, original length of sample = {len(data)}')
        # print(f'original length = {len(data)}')
        if len(data) >= self.num_samples:
            start_idx = np.random.randint(len(data) - self.num_samples + 1)

            np.append(peaks , [float("inf")])
            start_peak_idx = np.argmax(peaks >= start_idx)
            end_peak_idx = np.argmax(peaks >= start_idx + self.num_samples - 1)
            # print(start_idx,  peaks[start_peak_idx - 1],  peaks[start_peak_idx], peaks[end_peak_idx - 1], peaks[end_peak_idx], start_idx + self.num_samples - 1)

            data = data[start_idx : start_idx + self.num_samples]
            peaks = peaks[start_peak_idx : end_peak_idx] - start_idx
        else:
            left_pad = int(np.ceil((self.num_samples - len(data)) / 2))
            right_pad = int(np.floor((self.num_samples - len(data)) / 2))
            data = np.pad(data, (left_pad, right_pad), "constant")
            peaks += left_pad
        # print(peaks)
        return {"data": data, "label": label, "peaks": peaks}
